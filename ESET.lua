    ...
    if msg_type == 'MSG:Photo' then
      if not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
        ...
        if msg.content_.caption_ then
          ...
          if msg.content_.caption_:match(".[Ii][Rr]") or msg.content_.caption_:match(".[Cc][Oo][Mm]") or msg.content_.caption_:match(".[Oo][Rr][Gg]") or msg.content_.caption_:match(".[Ii][Nn][Ff][Oo]") or msg.content_.caption_:match("[Ww][Ww][Ww].") or msg.content_.caption_:match(".[Tt][Kk]") or msg.content_.ID == "MessageEntityTextUrl" or msg.content_.ID == "MessageEntityUrl" then
    ...
    elseif msg_type == 'MSG:Gif' then
      if not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
        ...
        if msg.content_.caption_ then
          ...
          if msg.content_.caption_:match(".[Ii][Rr]") or msg.content_.caption_:match(".[Cc][Oo][Mm]") or msg.content_.caption_:match(".[Oo][Rr][Gg]") or msg.content_.caption_:match(".[Ii][Nn][Ff][Oo]") or msg.content_.caption_:match("[Ww][Ww][Ww].") or msg.content_.caption_:match(".[Tt][Kk]") then
            if database:get('bot:webpage:mute'..msg.chat_id_) then
              local id = msg.id_
              local msgs = {[0] = id}
              local chat = msg.chat_id_
    ...
    elseif msg_type == 'MSG:Text' then
      ...
      if not is_free(msg, msg.content_.text_) then
        if not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
          ...
          if text:match(".[Ii][Rr]") or text:match(".[Cc][Oo][Mm]") or text:match(".[Oo][Rr][Gg]") or text:match(".[Ii][Nn][Ff][Oo]") or text:match("[Ww][Ww][Ww].") or text:match(".[Tt][Kk]") or msg.content_.ID == "MessageEntityTextUrl" or msg.content_.ID == "MessageEntityUrl" then
            if database:get('bot:webpage:mute'..msg.chat_id_) then
              local id = msg.id_
              local msgs = {[0] = id}
              local chat = msg.chat_id_
	 ...
	 local text = text:gsub('[!/#]','')
      if database:get('bot:cmds'..msg.chat_id_) and not is_momod(msg.sender_user_id_, msg.chat_id_) then
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) then
          if text:match("^[Pp]ing$") or text:match("^ربات$") then
            if database:get('lang:gp:'..msg.chat_id_) then
        ...
        -----------------------------------------------------------------------------------------------
        if text:match("^[Pp]romote @(.*)$") or text:match("^ارتقا مقام @(.*)$") and is_owner(msg.sender_user_id_, msg.chat_id_) then
          local ap = {string.match(text, "^([Pp]romote) @(.*)$")}
		  local ap = {string.match(text, "^ارتقا مقام @(.*)$")}
...
-------------------------
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) then
          if text:match("^[Ss]ettings$") or text:match("^تنظیمات$") then
			...
			TXTFA = TXTFA:gsub("#فعال","🔐")
			TXTEN = TXTEN:gsub("Active","🔐")
			TXTFA = TXTFA:gsub("#غیرفعال","🔓")
